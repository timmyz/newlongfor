    ...
    with app.app_context():
        ...
        try:
            ...
            if not user or not user.is_active:
                logger.warning(f"Job for user {user_id} is running, but user is not active or not found. Removing job.")
                remove_job_for_user(user_id)
                return

            logger.info(f"Running check-in for user: {user.username} ({user.account_id}) at scheduled time {user.checkin_time}")
            auth_data = {
                'token': user.token,
            ...
            db.commit()
            logger.info(f"User {user.username} check-in finished with status: {status}")

            # Send notification on failure
            if "失败" in status or "异常" in status:
                logger.warning(f"Check-in failed for {user.username}. Sending notification...")
            ...
            logger.error(f"An unexpected error occurred in job for user {user_id}: {e}")
        finally:
            db.close()
...
def add_or_update_job_for_user(user: crud.models.User, app: Flask):
    ...
    try:
        ...
        logger.info(f"Scheduled job for user {user.username} at {user.checkin_time} daily.")
    except Exception as e:
        logger.error(f"Failed to schedule job for user {user.id}: {e}")
...
def remove_job_for_user(user_id: int):
    ...
    if scheduler.get_job(job_id):
        ...
        logger.info(f"Removed job for user {user_id}.")
...
def initialize_scheduler(app: Flask):
    ...
    with app.app_context():
        db = SessionLocal()
        try:
            logger.info("Initializing scheduler and loading jobs from database...")
            active_users = db.query(crud.models.User).filter(crud.models.User.is_active == True).all()
            for user in active_users:
                add_or_update_job_for_user(user, app)
            logger.info(f"Scheduler initialized with {len(active_users)} jobs.")
        finally:
            db.close()
